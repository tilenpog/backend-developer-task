...
router.get("/noteitems", asyncHandler(async (req, res) => {
  const noteItems = await NoteController.getAllNoteItems(); //TODO: remove
  return ApiResponses.SUCCESS(res, noteItems);
}));

router.get("/", OptionalAuth, asyncHandler(async (req, res) => {
  const userId = req.authInfo.isAuthorized ? req.authInfo.user.id : -1;
  const notes = await NoteController.getAllNotes(userId);
...
router.post("/", RequireAuth, asyncHandler(async (req, res) => {
  ...
  const userOwnsFolder = await FolderController.userOwnsFolder(userId, createNoteData.folderId);
  if (!userOwnsFolder) {
...
router.put("/:id", RequireAuth, asyncHandler(async (req, res) => {
  ...
  const succeeded = await NoteController.updateNote(noteId, updateNoteData, userId);
